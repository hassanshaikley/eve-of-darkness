defmodule EOD.Web.ClientDetailChannel do
  ...
  def handle_in("watch_network", _, socket) do
    with {:ok, inspector} <- Inspector.start_link(),
         :ok <- Client.add_tcp_inspector(socket.assigns[:client_pid], inspector),
         subscription <- Inspector.SimpleSubscription.new() do
      Inspector.subscribe(inspector, subscription)
      {:noreply, socket}
    else
      error ->
        Logger.error("Error with `watch_network`: #{inspect error}")
        {:noreply, socket}
    end
  end
  ...
  def handle_info({:packet_inspection, dir, data}, socket) do
    IO.inspect data
    push(socket, "packet_traffic", %{dir: dir, data: "#{inspect data}"})
    {:noreply, socket}
  end
